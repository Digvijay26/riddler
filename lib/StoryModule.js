...
function StoryModule() {
  ...
  this.intentResponse = function(req, res) {
    ...
    if (req.session('intentName') === 'storyIntent') {
      ...
      return promise.then((successMessage) => {
        if (req.session('story').answers.includes(storyAnswer.toLowerCase())) {
          ...
          return res.say(prompt).reprompt(reprompt).session('puzzle_mode', 'story').shouldEndSession(false).send();
          ...
          return res.say(prompt).reprompt(reprompt).session('isAttemptLeft', false).session('puzzle_mode', 'story').shouldEndSession(false).send();
        }
    ...
    return new Promise((resolve, reject) => {
      ...
      return promise.then((successMessage) => {
        return res.say(prompt).reprompt(reprompt).session('story', result).session('intentName', 'storyIntent').session('isAttemptLeft', true).session('attempted_stories', attempted_stories).session('puzzle_mode', 'story').shouldEndSession(false);
      });
    }).catch((err) => {
  ...
  this.noIntentResponse = function(req, res) {
    ...
    //   });
    // } else {}
    if (req.session('story') === undefined || res.session('intentName') === 'launchIntent') {
      prompt = 'What would you like to play today? Riddles or story puzzles?';
      reprompt = 'Our options on Alexa are riddles and story puzzles. What would you like to play today?';
