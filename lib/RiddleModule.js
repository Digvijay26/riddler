...
function RiddleModule() {
  ...
  this.intentResponse = function(req, res) {
    ...
    if (req.session('intentName') === 'riddleIntent') {
      ...
      let promise = new Promise((resolve, reject) => {
        databaseHelper.saveUserState(deviceID, req.session('riddle').riddle_id, req.session('riddle'), req.session('riddle').attempted_riddles, 'riddle', (err, userResult) => {
      ...
      return promise.then((successMessage) => {
        ...
        prompt = `<say-as interpret-as="interjection">${RandomModule[wrongAnswerExpression][ Math.floor(Math.random() * RandomModule[wrongAnswerExpression].length) ]}</say-as> The answer is, ${req.session('riddle').riddle_answer} <break time=\"2.00s\"/>  ${RandomModule.riddleAgain[ Math.floor(Math.random() * RandomModule.riddleAgain.length) ]}`;
        reprompt = 'Would you like to listen to another riddle?';
        return res.say(prompt).reprompt(reprompt).shouldEndSession(false).send();
      });
    }
    return new Promise((resolve, reject) => {
      ...
      reprompt = 'If you want to repeat the riddle, just say, repeat. If you want to go to the next riddle or dont know the answer, just say, pass or next.';

      let promise = new Promise((resolve, reject) => {
        databaseHelper.saveUserState(deviceID, result.riddle_id, result, attempted_riddles, 'riddle', (err, userResult) => {
          if (err) return reject(err);
          return resolve(userResult);
        });
      });
  ...
  this.yesIntentResponse = function(req, res) {
    ...
    if (res.session('intentName') === 'launchIntent' && req.session('riddle') !== undefined) {
      ...
      const randomRiddleId = GenericModule.getRiddleId(res.session('attempted_riddles')) || null;

      if (randomRiddleId === undefined) {
        prompt = 'You have completed all riddles of this week. Please wait for another week for new set of riddles. Meanwhile, would you like to solve some mystery stories?';
  ...
  this.noIntentResponse = function(req, res) {
    ...
    } else if (req.session('riddle') === undefined) {
      prompt = 'What would you like to play today? Riddles, question of the day or story puzzles?';
      reprompt = 'Our options on Alexa are riddles, question of the day and story puzzles. What would you like to play today?';
