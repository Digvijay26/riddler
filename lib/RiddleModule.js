  ...
  this.intentResponse = function(req, res) {
    ...
    if (req.session('intentName') === 'riddleIntent') {
      ...
      return promise.then((successMessage) => {
        console.log('user answer:', riddleAnswer.toLowerCase());
        console.log('correct answer:', req.session('riddle').answers);
        console.log('answer present in array:', req.session('riddle').answers.includes(riddleAnswer.toLowerCase()));
        if (req.session('riddle').answers.includes(riddleAnswer.toLowerCase())) {
          ...
          prompt = `<say-as interpret-as="interjection">${RandomModule[wrongAnswerExpression][ Math.floor(Math.random() * RandomModule[wrongAnswerExpression].length) ]}</say-as>, that\'s not the correct answer. ${RandomModule.answerChange[ Math.floor(Math.random() * RandomModule.answerChange.length) ]}. If you need help, you can ask for hint. To answer the riddle, just say - Alexa, My answer is, followed by your answer.`;
    ...
    return new Promise((resolve, reject) => {
      ...
      const startPrompt = `${RandomModule.launch[ Math.floor(Math.random() * RandomModule.launch.length) ]}  and welcome to the temple of puzzles. Here goes your riddle. <break time=\"2.00s\"/>`;
