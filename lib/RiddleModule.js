...
function RiddleModule() {
  ...
  this.intentResponse = function(req, res) {
    ...
    if (req.session('intentName') === 'riddleIntent') {
      ...
      return promise.then((successMessage) => {
        if (req.session('riddle').answers.includes(riddleAnswer.toLowerCase())) {
          ...
          prompt = `<say-as interpret-as="interjection">${RandomModule[wrongAnswerExpression][ Math.floor(Math.random() * RandomModule[wrongAnswerExpression].length) ]}</say-as> The answer is, ${req.session('riddle').riddle_answer} ${RandomModule.riddleAgain[ Math.floor(Math.random() * RandomModule.riddleAgain.length) ]}`;
          reprompt = 'Would you like to listen to another riddle?'
          return res.say(prompt).reprompt(reprompt).shouldEndSession(false).send();
        }
      });
    ...
    return new Promise((resolve, reject) => {
      ...
      const staryPrompt = `${RandomModule.launch[ Math.floor(Math.random() * RandomModule.launch.length) ]}  and welcome to Riddler. Here goes your riddle. <break time=\"3.00s\"/>`;
